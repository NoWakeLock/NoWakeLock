...
data class BottomNavItem(
    val route: String, val titleResId: Int, val icon: @Composable () -> Unit
)
...
fun NoWakeLockBottomNavBar(navController: NavController) {
    val items = listOf(
        ...
        icon = { Icon(Icons.Rounded.Apps, contentDescription = null) }), BottomNavItem(
        route = NavRoutes.WAKELOCKS,
        titleResId = R.string.WakeLock,
        icon = { Icon(Icons.Rounded.Lock, contentDescription = null) }), BottomNavItem(
        route = NavRoutes.ALARMS,
        titleResId = R.string.Alarm,
        icon = { Icon(Icons.Rounded.Alarm, contentDescription = null) }), BottomNavItem(
        route = NavRoutes.SERVICES,
        titleResId = R.string.Service,
        icon = { Icon(Icons.Rounded.Build, contentDescription = null) }), BottomNavItem(
    ...
    NavigationBar {
        items.forEach { item ->
            ...
            NavigationBarItem(icon = item.icon, label = {
                Text(
                    stringResource(id = item.titleResId),
                    style = MaterialTheme.typography.labelLarge, // 文字大小
