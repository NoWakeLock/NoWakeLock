...
## [v3.0.11 Build 89] - 测试版准备（2026-09-12）

本版本通过 GitHub **Pre-release** 渠道试用，暂不作为稳定版推广。

### 新增与改进

- 保留旧 Xposed 入口，新增独立的官方 API 102 路径，适配唤醒锁、闹钟和 Service；中间过渡 API 不在兼容目标内。
- 规则原子替换、实时发布并确认生效，无需重启或应用常驻；统计改为有界队列和后台批量写入，提供丢弃/失败诊断。
- 支持 API 102 手动及 APK 更新触发的模块代码热重载，交接规则、统计和活跃唤醒锁，检查实际运行的构建身份。

### 修复

- 空正则组导致规则保存后未发布；统计读取或本地缓存失败时误清空系统端统计。
- 现代 Service 嵌套重载重复计数；首次阻断唤醒锁后的时长虚增；旧框架阻断 bindService 的返回值异常。

### 升级与验证范围

- 升级前备份规则。同签名 v3.x 可覆盖安装；本地调试签名切换到 CI 签名可能需要备份后重装。
- 首次从不具备热重载能力的构建升级可能仍需重启；后续热重载依赖框架支持，旧框架仍按原方式重启加载新 APK。
- 前序 3.0.10 / 88 候选通过 OP13 / Vector 2.2 的手动和自动重载、跨代持锁、用户 0/10 的放行/阻断/恢复，以及 120 项单元测试。
- 当前最终实现的旧框架整机、独立 Provider、首次解锁前和部分重载并发边界仍待测；前序候选证据不能当作新 CI 产物的安装验收。
- [完整中英更新说明与测试限制](https://github.com/NoWakeLock/NoWakeLock/blob/feature/dual-xposed-config-backend/fastlane/release-notes/3.0.11.md)

---

## [v3.0.10 Build 87] - 2026-05-17

- 修复应用和事件列表的排序/筛选数据流，补充排序相关单元与设备测试。
- 后续本地兼容性候选曾沿用 3.0.10，并递增至 Build 88；这些开发验证统一归入上述 3.0.11 测试版说明。
...
### 🐛 修复
- 修复同名 WakeLock/Alarm 在不同应用间共享最小间隔状态，避免跨应用误拦截
- 修复 Service 域“仅灭屏拦截”规则未生效的问题
- 修复窄宽度下控制区超时输入框被裁切的问题

### 🧪 测试与质量
- 补齐 `calculateTime`、Backup JSON、Preferences 映射、Wakelock counter/registry 的单元测试
- 新增 MainActivity、`XProvider` 和窄宽布局的 instrumentation 测试
- 固化真机自动化测试脚本与执行文档
