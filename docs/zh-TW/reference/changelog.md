...
## [v3.0.11 Build 89] - 測試版準備（2026-09-12）

本版本透過 GitHub **Pre-release** 管道試用，暫不作為穩定版推廣。

### 新增與修復

- 保留舊 Xposed 入口，新增獨立的官方 API 102 路徑，適配喚醒鎖、鬧鐘與 Service；中間過渡 API 不在相容目標內。
- 原子替換規則並確認即時生效，不需重啟或應用常駐；統計改為有界佇列與背景批次寫入，提供丟棄/失敗診斷。
- 支援 API 102 手動與 APK 更新觸發的模組熱重載，交接規則、統計及活躍喚醒鎖，核對實際執行的建置身分。
- 修復空正則群組發布失敗、讀取/快取失敗時誤清統計、現代 Service 重載重複計數、首次阻斷後時長虛增及舊框架 bindService 阻斷回傳值異常。

### 升級與驗證範圍

- 先備份規則。同簽章 v3.x 可覆蓋安裝；本地除錯簽章切換至 CI 簽章可能需備份後重裝。
...
- [完整中英更新說明與測試限制](https://github.com/NoWakeLock/NoWakeLock/blob/dev/fastlane/release-notes/3.0.11.md)

---

## [v3.0.10 Build 87] - 2026-05-17

- 修復應用與事件列表排序/篩選資料流，補充排序相關單元與裝置測試。
- 後續本地相容性候選沿用 3.0.10 至 Build 88；相關變更統一記入上述 3.0.11 測試版。
...
### 🐛 修復
- 修復 Android 16 多使用者環境下「應用列表」崩潰
- 增強 Samsung/OneUI 相容性（OEM fallback + Nest accessor 鉤子支援）
- LSPosed 推薦作用域移除 `com.android.settings.provider`，避免誤導
- 縮短 fastlane metadata 的 `short_description`（避免被截斷/產生警告）
...
### 🔧 改進
- 優化 GitHub Actions 發版流程（tag/version 處理）
...
### ✨ 新功能
- 增加 F-Droid 可重複構建支援（reproducible builds）
- APK 命名調整為 `NoWakeLock-%v.apk`（F-Droid 相容）
- 文件更新：Android 相容性說明更新至 16
